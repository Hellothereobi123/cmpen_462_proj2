# cmpen_462_proj2

This mini-project 2 was developed my Tharun Dilliraj and Brandon Widjaja
4/8/26
github repo = https://github.com/Hellothereobi123/cmpen_462_proj2

# answer to sample input 
The 3d velocity is [2.96246152 1.97989947 0.99733742] and the speed is 3.7001164939722546

# prep
Prep (no venv required):
download Python 3.14.3 or higher
download VScode (optional)

make sure pip is installed, then 
    pip install numpy 
    pip install scipy 



# In order to to use our program,
edit the input file according to your specifications 
    first line:     input the clients 3d location
    second line:    input the number of access points 
    third line +:   input each access point's 3d location

import the AP files into the ~/cmpen_462_proj2 folder 

After, on vscode, hit run. ( if you are using CMDline, cd to the program location, then type "python MP2.py" to run" )

